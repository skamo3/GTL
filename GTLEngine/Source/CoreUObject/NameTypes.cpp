...
public:
	...
	{
		return DisplayPool.find(Hash)->second;
	}
	...
	FNameEntryId FindOrStoreString(const FNameStringView& Name)
	{
		...
		if (DisplayPool.find(DisplayValue.Hash) != DisplayPool.end())
		{
			return {DisplayValue.Hash};
		}

		const FNameComparisonValue ComparisonValue{Name};
		if (!ComparisonPool.contains(ComparisonValue.Hash))
		{
			const FNameEntry Entry = MakeEntry(ComparisonValue);
			ComparisonPool.insert({ ComparisonValue.Hash, Entry });
		}

		DisplayValue.ComparisonId = {ComparisonValue.Hash};
		DisplayPool.insert({ DisplayValue.Hash, MakeEntry(DisplayValue) });
		return {DisplayValue.Hash};
	}
};
...
struct FNameHelper
{
	...
	static FName MakeFName(const FString& Name, uint32 Len)
	{
		return MakeFName(Name.c_str(), Len);
	}
